  ...
  switch (type) {
    case "education":
      validSchema = {
        ...
        fieldOfStudy:
          req.method === "POST" ? Joi.string().required() : Joi.string(),
      };
      break;
...
const validCareerSchema = (type, req, maxAllowed) =>
  Joi.object().keys(getValidCareerSchema(type, req)).max(maxAllowed);
    ...
    const schema = {
      name: req.url.split("/").includes("register")
        ? Joi.string()
            ...
            .error(
              new Error(
                '"Name" of at least 2 and at max 50 letters is required!'
              )
            )
