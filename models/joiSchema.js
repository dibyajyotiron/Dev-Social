...
const validCareerSchema = (type, req, maxAllowed) => Joi.object().keys(getValidCareerSchema(type, req)).max(maxAllowed);

module.exports = {
	validateUserSchema(user, req) {
		const schema = {
			name: req.url.split("/").includes("register") ? Joi.string().min(2).max(50).required().error(new Error('"Name" of at least 2 and at max 50 letters is required!')) : Joi.forbidden().error(new Error("Name is not allowed!")),
	...
	validateComment(comment) {
		const schema = {
			text: Joi.string().min(2).max(150),
			name: Joi.string().min(2).max(25),
		};
		return Joi.validate(comment, schema);
	},
	validateUserFollowSchema(follow) {
		const schema = {
			_follows: Joi.objectId().required(),
		};
